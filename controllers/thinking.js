...
exports.thinking_create = function (req, res) {
    ...
    // 确保找到对应的作者
    UserModel.findOne({code : req.body.code }, function (err, user) {
        if (err) {
            res.send(err);
        } else {
            ...
            thinking.save(function (err) {
                ...
                user.save(function(err, user){
                    if (err) {
                        ...
                        res.send('Thinking Created successfully by '+user.code+" at "+thinking.datestr+" & "+user.isdone);
                    }
                });
            });
        } 
...
exports.thinking_get_by_user_date = function (req, res) {
    ThinkingModel.findOne({code : req.params.code, datestr : req.params.datestr}, function (err, thinking) {
        if (err) res.send(err);
        res.send(thinking);
    });
};
...
exports.thinking_get_all_thinkings= function (req, res) {
    ThinkingModel.find({}, function (err, thinking) {
        if (err) res.send(err);
        res.send(thinking);
    });
};
